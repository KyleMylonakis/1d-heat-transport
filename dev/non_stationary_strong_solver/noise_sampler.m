...
T_inv = zeros(particles);
parfor j = 1:particles
    if mod(j,100) == 0
	j
    end
    for i = 1:particles
        T_inv(i,j) = -( (i + j - abs(j-i))*(2*particles + 2 - abs(j-i) -i - j))/(4*(particles + 1));
        %T_inv(i,j) = -( (i + j - abs(j-i))*(2*particles + 2 - abs(j-i) -i - j));
    end
end
%T_inv = T_inv./(4*(particles + 1));

%T = full(gallery('tridiag',particles,1,-2,1)); 

%T*T_inv;
%T_inv*T

%A = full(gallery('tridiag',particles,-1*k_sqrd,2*k_sqrd,-1*k_sqrd)); 
%B = (Ltemp./(-k_sqrd)).*T_inv;
...
%max(max(left_pos_covariance - B))
%left_pos_sample = mvnrnd(mean, left_pos_covariance, total_samples);
left_pos_covariance = (Ltemp./(-k_sqrd)).*T_inv;
left_pos_sample = mvnrnd(mean, left_pos_covariance, total_samples);
...
right_pos_covariance = (Rtemp./(-k_sqrd)).*T_inv;
...
% Remove T_inv from memory T_inv
clear T_inv;
