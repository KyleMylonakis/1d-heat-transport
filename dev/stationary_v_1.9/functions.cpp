...
void f_system(double *pos, double *first_vel, double *last_vel, double* int_velocities, long long int current_step, double *theta, double left_noise, double right_noise, double theta_norm)
{
    ...
    temp_first_vel = fpot(pos[1] - pos[0] + a0)  + ( (g_k_sqrd * left_noise) ) -g_k_sqrd*trap(first_vel,theta, current_step, theta_norm)*g_dt ;
    for (int j = 1; j < n_int_particles+1; ++j)
    {
        temp_int_velocities[j-1] = fpot(pos[j+1] - pos[j] + a0) -fpot(pos[j] - pos[j -1] + a0);
    }

    temp_last_vel = -fpot(pos[n_particles -1] - pos[n_particles -2] + a0) + ( (g_k_sqrd * right_noise) ) -g_k_sqrd*trap(last_vel,theta, current_step, theta_norm)*g_dt ;
...
void f_system_finer_grids(double *pos, double *first_vel, double *last_vel, double* int_velocities, long long int current_step, double *theta, double* left_noise, double* right_noise, double theta_norm, double alter_step_size)
{
    ...
    */
    ...
    temp_first_vel = fpot(pos[1] - pos[0] + a0) + ( (g_k_sqrd * left_noise[current_step - 1]) ) -g_k_sqrd*trap(first_vel,theta, current_step, theta_norm)*g_dt*alter_step_size ;
    for (int j = 1; j < n_int_particles+1; ++j)
    {
        temp_int_velocities[j-1] = fpot(pos[j+1] - pos[j] + a0) -fpot(pos[j] - pos[j -1] + a0);
    }
    temp_last_vel = -fpot(pos[n_particles -1] - pos[n_particles -2] + a0) + ( (g_k_sqrd * right_noise[current_step - 1]) / sqrt(theta_norm) ) -g_k_sqrd*trap(last_vel,theta, current_step, theta_norm)*g_dt*alter_step_size ;
                    ...
                    double hold_first_vel, double hold_last_vel, double *shifted_theta)
{
    ...
    temp_first_vel = fpot(pos[1] - pos[0] + a0) + ( (g_k_sqrd * left_noise) ) -g_k_sqrd*trap_xtra(extern_temp_first_vel,theta, current_step, theta_norm, coeff, xtra_step_size, shifted_theta)*g_dt ;
    ...
    temp_last_vel = -fpot(pos[n_particles -1] - pos[n_particles -2] + a0) + ( (g_k_sqrd * right_noise)  ) - g_k_sqrd*trap_xtra(extern_temp_last_vel,theta, current_step, theta_norm, coeff, xtra_step_size, shifted_theta)*g_dt ;
...
double *theta, double* left_noise, double* right_noise, double theta_norm, double *shifted_theta)
{
    ...
    f_system( pos, first_vel, last_vel, int_velocities, current_step, theta, left_noise[2*(current_step - 1)], right_noise[2*(current_step - 1)], theta_norm);
    ...
    */
    ...
    f_system_xtra( beta_pos, first_beta_vel, last_beta_vel, beta_vel, current_step, theta, left_noise[2*(current_step)], right_noise[2*(current_step)], theta_norm, xtra_step_size, first_vel, last_vel, hold_first_vel, hold_last_vel, shifted_theta);
